...
def skip_wav_seconds(wav_file, seconds, rate):
    total_frames = wav_file.getnframes()  # Total de frames en el archivo
    duration_seconds = total_frames / rate  # Duración total en segundos

    print(f"Duración total del archivo: {duration_seconds:.2f} segundos")  # Imprimir la duración

    frames_to_skip = int(seconds * rate)
    print(f"Frames a saltar: {frames_to_skip}, Total de frames: {total_frames}")

    # Verificar si el número de frames a saltar está dentro del rango permitido
    if frames_to_skip > total_frames:
        raise ValueError(
            f"Intentando saltar más frames de los disponibles. Se solicitó saltar {frames_to_skip} frames, pero el archivo solo tiene {total_frames} frames.")
...
try:
    ...
    for time_idx in range(0, int(RATE / CHUNK * RECORD_SECONDS)):
        ...
        print(
            f"Tiempo: {current_time:.2f} s - Ángulo estimado: Azimut = {estimated_azimuth:.2f}°, Elevación = {estimated_elevation:.2f}°")
