...
def simulate_drone_flight_singleband_no_pitch(
    ...
    The delay is computed using a planar wavefront assumption. The arrival direction is calculated as
    the negative of the flight position vector (relative to the array), so that the delays reflect the
    correct direction from the source (drone) to the array.
    """
    # 1) Load microphone positions and flight positions
    mic_positions = initialize_microphone_positions_24()  # Expected shape: (24, 3)
    # Compute flight positions relative to reference (in meters) with the same projection as beamforming
    ...
    for idx in tqdm(range(n_positions), desc="Simulating flight (no Doppler shift)"):
        ...
        for mic_idx in range(n_mics):
            # Compute the unit arrival direction vector (from drone to array) assuming far-field.
            # Invert the flight position vector to get the direction from the source to the array.
            norm_flight = np.linalg.norm(flight_positions[idx]) + epsilon
            unit_direction = -flight_positions[idx] / norm_flight

            # Compute delay using the dot product (projection) of the mic position onto the unit direction
            delay = np.dot(mic_positions[mic_idx], unit_direction) / sound_speed
            t_arrival = t_emit + delay
            sample_arrival = int(t_arrival * sample_rate)

            # Adjust for negative sample arrival (if the computed delay is negative)
            if sample_arrival < 0:
            ...
            # Calculate the available length in the simulation buffer from sample_arrival
            available_length = n_samples_total - sample_arrival
            seg_len = min(len(effective_segment), available_length)

            if seg_len > 0:
                # Compute the Euclidean distance for attenuation calculation
                ...
                # Update the microphone signal with the delayed and attenuated segment
                mic_signals[mic_idx, sample_arrival:sample_arrival+seg_len] += effective_segment[:seg_len] * att_factor
