...
audioBuffers = [[bytearray(AUDIO_BUFFER_SIZE), bytearray(AUDIO_BUFFER_SIZE)],[bytearray(AUDIO_BUFFER_SIZE), bytearray(AUDIO_BUFFER_SIZE)],[bytearray(AUDIO_BUFFER_SIZE), bytearray(AUDIO_BUFFER_SIZE)],]

audioBufferLock = [threading.Lock(), threading.Lock(), threading.Lock()]
audioBufferReady = [threading.Condition(audioBufferLock[0]),threading.Condition(audioBufferLock[1]),threading.Condition(audioBufferLock[2]),]
...
def receiveSensorData(sensorTemp, sensorsFileNumber, deviceName):
    ...
    with open(accelerometer_file, mode="w", newline="") as acc_file, open(gyroscope_file, mode="w", newline="") as gyro_file, open(magnetometer_file, mode="w", newline="") as mag_file, open(temperature_file, mode="w", newline="") as temp_file:
        # Create CSV writers
                        ...
                        if isinstance(sensor_data_list, list):
                            for sensor_data in sensor_data_list:
                                ...
                                if "accX" in sensor_data and "accY" in sensor_data and "accZ" in sensor_data:
                                    acc_writer.writerow([sensor_data["accX"], sensor_data["accY"],sensor_data["accZ"],])
                                
                                if "gyroX" in sensor_data and "gyroY" in sensor_data and "gyroZ" in sensor_data:
                                    gyro_writer.writerow([sensor_data["gyroX"], sensor_data["gyroY"], sensor_data["gyroZ"],])
                                
                                if ("magX" in sensor_data and "magY" in sensor_data and "magZ" in sensor_data ):
                                    mag_writer.writerow([sensor_data["magX"],sensor_data["magY"],sensor_data["magZ"],])
...
"""
def recordAudioOverNetwork(masterDeviceNetworkForAudio, slaveDeviceNetworkForAudio,slaveDeviceNetworkForAudioDevNum2,numberOfAudioFilesToBeRecorded,masterDeviceIPForAudio,slaveDeviceIPForAudio,slaveDeviceIPForAudioDevNum2):
    currentTimeOfSendingCommand = datetime.now().strftime("%Y%m%d_%H%M%S")
    generateJson(masterDeviceNetworkForAudio,slaveDeviceNetworkForAudio,slaveDeviceNetworkForAudioDevNum2,masterDeviceIPForAudio,slaveDeviceIPForAudio,slaveDeviceIPForAudioDevNum2,placeHolder,currentTimeOfSendingCommand)
    ...
    if numberOfDevicesConnected == 1:  # master device only
      ...
      threadTocompileAudioDataIntoWavFileMasterDevice = threading.Thread(target=compileAudioDataIntoWavFile,args=(masterDeviceNetworkForAudio, numberOfAudioFilesToBeRecorded, "Master", 0),)
      ...
      # create thread to send audio
      threadTocompileAudioDataIntoWavFileMasterDevice = threading.Thread(target=compileAudioDataIntoWavFile,args=(masterDeviceNetworkForAudio, numberOfAudioFilesToBeRecorded, "Master", 0),)
      threadTocompileAudioDataIntoWavFileSlaveDevice = threading.Thread(target=compileAudioDataIntoWavFile,args=(slaveDeviceNetworkForAudio, numberOfAudioFilesToBeRecorded, "Slave", 1),)
      ...
      threadTocompileAudioDataIntoWavFileMasterDevice = threading.Thread(target=compileAudioDataIntoWavFile,args=(masterDeviceNetworkForAudio, numberOfAudioFilesToBeRecorded, "Master", 0),)
      threadTocompileAudioDataIntoWavFileSlaveDevice = threading.Thread(target=compileAudioDataIntoWavFile,args=(slaveDeviceNetworkForAudio, numberOfAudioFilesToBeRecorded, "Slave", 1),)
      threadTocompileAudioDataIntoWavFileSlaveDeviceNum2 = threading.Thread(target=compileAudioDataIntoWavFile,args=(slaveDeviceNetworkForAudioDevNum2, numberOfAudioFilesToBeRecorded, "Slave_device_number_2", 2),)
      threadTocompileAudioDataIntoWavFileSlaveDevice.start()
...
""" @brief Records sensor data over network, either wifi or ethernet
    ...
    @parma slaveDeviceNetwork can be either slaveDeviceEthernet or slaveDeviceWiFi
"""
def recordSensorDataOverNetwork(masterDeviceNetworkForSensor, slaveDeviceNetworkForSensor,slaveDeviceNetworkForSensorDevNum2,numberOfSensorFilesToBeRecorded,masterDeviceIPForSensors,slaveDeviceIPForSensors,slaveDeviceIPForSensorsDevNum2):
  currentTimeOfSendingCommand = datetime.now().strftime("%Y%m%d_%H%M%S")
  generateJson(masterDeviceNetworkForSensor,slaveDeviceNetworkForSensor,slaveDeviceNetworkForSensorDevNum2,masterDeviceIPForSensors,slaveDeviceIPForSensors,slaveDeviceIPForSensorsDevNum2,placeHolder,currentTimeOfSendingCommand)
  if numberOfDevicesConnected == 1:  # master device only
    threadToReceiveSensorDataAndRecordToCSVonMasterDevice = threading.Thread(target=receiveSensorData,args=(masterDeviceNetworkForSensor,numberOfSensorFilesToBeRecorded,"Master",),)
    ...
    threadToReceiveSensorDataAndRecordToCSVonMasterDevice = threading.Thread(target=receiveSensorData,args=(masterDeviceNetworkForSensor,numberOfSensorFilesToBeRecorded,"Master",),)
    threadToReceiveSensorDataAndRecordToCSVonSlaveDevice = threading.Thread(target=receiveSensorData,args=(slaveDeviceNetworkForSensor,numberOfSensorFilesToBeRecorded,"Slave",),)
    threadToReceiveSensorDataAndRecordToCSVonMasterDevice.start()
    ...
    threadToReceiveSensorDataAndRecordToCSVonMasterDevice = threading.Thread(target=receiveSensorData,args=(masterDeviceNetworkForSensor,numberOfSensorFilesToBeRecorded,"Master",),)
    threadToReceiveSensorDataAndRecordToCSVonSlaveDevice = threading.Thread(target=receiveSensorData,args=(slaveDeviceNetworkForSensor,numberOfSensorFilesToBeRecorded,"Slave",),)
    threadToReceiveSensorDataAndRecordToCSVonSlaveDeviceNum2 = threading.Thread(target=receiveSensorData,args=(slaveDeviceNetworkForSensorDevNum2,numberOfSensorFilesToBeRecorded,"Slave_device_number_2",),)
    threadToReceiveSensorDataAndRecordToCSVonMasterDevice.start()
    threadToReceiveSensorDataAndRecordToCSVonSlaveDevice.start()
...
def recordData(tempSelectedRecordingMethodValue, tempSelectedDataToRecordValue):
    ...
    if tempSelectedRecordingMethodValue == "1" and tempSelectedDataToRecordValue == "1":
        ...
        for i in range(numberOfFilesToRecord):
          recordAudioOverNetwork(masterDeviceEthernet, slaveDeviceEthernet,slaveDeviceEthernetDeviceNumber2,i,DEVICE_0_ETHERNET_IP,DEVICE_1_ETHERNET_IP,DEVICE_2_ETHERNET_IP)
        ...
        for i in range(numberOfFilesToRecord):
          recordSensorDataOverNetwork(masterDeviceEthernet, slaveDeviceEthernet,slaveDeviceEthernetDeviceNumber2,i,DEVICE_0_ETHERNET_IP,DEVICE_1_ETHERNET_IP,DEVICE_2_ETHERNET_IP)
          restartEthernetConnection(2)
    ...
    # ----------------------------------------------------------------------------------------------------------------------#
        ...
        for i in range(numberOfFilesToRecord):
          recordAudioOverNetwork(masterDeviceWiFi, slaveDeviceWiFi,slaveDeviceWiFiDeviceNumber2,i,DEVICE_0_WIFI_IP,DEVICE_1_WIFI_IP,DEVICE_2_WIFI_IP)
        ...
        for i in range(numberOfFilesToRecord):
          recordSensorDataOverNetwork(masterDeviceWiFi, slaveDeviceWiFi,slaveDeviceWiFiDeviceNumber2,i,DEVICE_0_WIFI_IP,DEVICE_1_WIFI_IP,DEVICE_2_WIFI_IP)
          restartWiFiConnection(2)
    ...
    # ----------------------------------------------------------------------------------------------------------------------#
        ...
        generateJson(masterDeviceEthernet,slaveDeviceEthernet,slaveDeviceEthernetDeviceNumber2,DEVICE_0_ETHERNET_IP,DEVICE_1_ETHERNET_IP,DEVICE_2_ETHERNET_IP,placeHolder,currentTimeOfSendingCommandForSDrecording)
    ...
    # ----------------------------------------------------------------------------------------------------------------------#
        ...
        if numberOfDevicesConnected == 1:           # master device only      
            for i in range(numberOfFilesToRecord):  # Record Audio to SD card
                ...
                generateJson(masterDeviceWiFi,slaveDeviceWiFi,slaveDeviceWiFiDeviceNumber2,DEVICE_0_WIFI_IP,DEVICE_1_WIFI_IP,DEVICE_2_WIFI_IP,i,currentTimeOfSendingCommand)
            ...
            for i in range(numberOfFilesToRecord): # Download recorded audio on SD card
                ...
                # create thread
                threadToautodownloadAudioOnSDcardMasterDevice = threading.Thread(target=autodownloadAudioOnSDcard,args=(masterDeviceWiFi,fileNumberToDownloadString,"master_device",),)
                threadToautodownloadAudioOnSDcardSlaveDevice = threading.Thread(target=autodownloadAudioOnSDcard,args=(slaveDeviceWiFi,fileNumberToDownloadString,"slave_device",),)
            ...
            for i in range(numberOfFilesToRecord): # Download recorded audio on SD card
                ...
                threadToautodownloadAudioOnSDcardMasterDevice = threading.Thread(target=autodownloadAudioOnSDcard,args=(masterDeviceWiFi,fileNumberToDownloadString,"master_device",),)
                threadToautodownloadAudioOnSDcardSlaveDevice = threading.Thread(target=autodownloadAudioOnSDcard,args=(slaveDeviceWiFi,fileNumberToDownloadString,"slave_device",),)
                threadToautodownloadAudioOnSDcardSlaveDeviceNum2 = threading.Thread(target=autodownloadAudioOnSDcard,args=(slaveDeviceWiFiDeviceNumber2,fileNumberToDownloadString,"slave_device_number_2",),)
        ...
        if numberOfDevicesConnected == 1:  # master device only      
            ...
            for i in range(numberOfFilesToRecord): # download data recorded
                ...
                # create thread
                threadToautodownloadAccelerometerDataOnSDcard = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(masterDeviceWiFi,fileNumberToDownloadString,"accelerometer","master_device",),)
                ...
                # create thread
                threadToautodownloadGyroscopeDataOnSDcard = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(masterDeviceWiFi,fileNumberToDownloadString,"gyroscope","master_device",),)
                ...
                # create thread
                threadToautodownloadMagnetometerDataOnSDcard = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(masterDeviceWiFi,fileNumberToDownloadString,"magnetometer","master_device",),)
                ...
                #create thread
                threadToautodownloadTemperatureDataOnSDcard = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(masterDeviceWiFi,fileNumberToDownloadString,"temperature","master_device",),)
            ...
            for i in range(numberOfFilesToRecord): # download data recorded
                ...
                # create threads
                threadToautodownloadAccelerometerDataOnSDcardMasterDevice = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(masterDeviceWiFi,fileNumberToDownloadString,"accelerometer","master_device",),)
                threadToautodownloadAccelerometerDataOnSDcardSlaveDevice = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(slaveDeviceWiFi,fileNumberToDownloadString,"accelerometer","slave_device",),)
                ...
                # create threads
                threadToautodownloadGyroscopeDataOnSDcardMasterDevice = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(masterDeviceWiFi,fileNumberToDownloadString,"gyroscope","master_device",),)
                threadToautodownloadGyroscopeDataOnSDcardSlaveDevice = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(slaveDeviceWiFi,fileNumberToDownloadString,"gyroscope","slave_device",),)
                ...
                # create threads
                threadToautodownloadMagnetometerDataOnSDcardMasterDevice = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(masterDeviceWiFi,fileNumberToDownloadString,"magnetometer","master_device",),)
                threadToautodownloadMagnetometerDataOnSDcardSlaveDevice = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(slaveDeviceWiFi,fileNumberToDownloadString,"magnetometer","slave_device",),)
                ...
                # create threads
                threadToautodownloadTemperatureDataOnSDcardMasterDevice = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(masterDeviceWiFi,fileNumberToDownloadString,"temperature","master_device",),)
                threadToautodownloadTemperatureDataOnSDcardSlaveDevice = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(slaveDeviceWiFi,fileNumberToDownloadString,"temperature","slave_device",),)
            ...
            for i in range(numberOfFilesToRecord): # download data recorded
                ...
                threadToautodownloadAccelerometerDataOnSDcardMasterDevice = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(masterDeviceWiFi,fileNumberToDownloadString,"accelerometer","master_device",),)
                threadToautodownloadAccelerometerDataOnSDcardSlaveDevice = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(slaveDeviceWiFi,fileNumberToDownloadString,"accelerometer","slave_device",),)
                threadToautodownloadAccelerometerDataOnSDcardSlaveDeviceNumber2 = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(slaveDeviceWiFiDeviceNumber2,fileNumberToDownloadString,"accelerometer","slave_device_number_2",),)
                ...
                threadToautodownloadGyroscopeDataOnSDcardMasterDevice = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(masterDeviceWiFi,fileNumberToDownloadString,"gyroscope","master_device",),)
                threadToautodownloadGyroscopeDataOnSDcardSlaveDevice = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(slaveDeviceWiFi,fileNumberToDownloadString,"gyroscope","slave_device",),)
                threadToautodownloadGyroscopeDataOnSDcardSlaveDeviceNumber2 = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(slaveDeviceWiFiDeviceNumber2,fileNumberToDownloadString,"gyroscope","slave_device_number_2",),)
                ...
                threadToautodownloadMagnetometerDataOnSDcardMasterDevice = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(masterDeviceWiFi,fileNumberToDownloadString,"magnetometer","master_device",),)
                threadToautodownloadMagnetometerDataOnSDcardSlaveDevice = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(slaveDeviceWiFi,fileNumberToDownloadString,"magnetometer","slave_device",),)
                threadToautodownloadMagnetometerDataOnSDcardSlaveDeviceNumber2 = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(slaveDeviceWiFiDeviceNumber2,fileNumberToDownloadString,"magnetometer","slave_device_number_2",),)
                ...
                threadToautodownloadTemperatureDataOnSDcardMasterDevice = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(masterDeviceWiFi,fileNumberToDownloadString,"temperature","master_device",),)
                threadToautodownloadTemperatureDataOnSDcardSlaveDevice = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(slaveDeviceWiFi,fileNumberToDownloadString,"temperature","slave_device",),)
                threadToautodownloadTemperatureDataOnSDcardSlaveDeviceNumber2 = threading.Thread(target=autodownloadSensorDataOnSDcard,args=(slaveDeviceWiFiDeviceNumber2,fileNumberToDownloadString,"temperature","slave_device_number_2",),)
      ...
      if numberOfDevicesConnected == 1:  # master device only      
        ...
        for i in range(numberOfFilesToRecord): # download data recorded
          ...
          threadToautodownloadAudioOnSDcardMasterDevice = threading.Thread(target=autodownloadAudioOnSDcard,args=(masterDeviceWiFi,fileNumberToDownloadString,"master_device",),)
          threadToautodownloadAudioOnSDcardSlaveDevice = threading.Thread(target=autodownloadAudioOnSDcard,args=(slaveDeviceWiFi,fileNumberToDownloadString,"slave_device",),)
          threadToautodownloadAudioOnSDcardSlaveDeviceNumber2 = threading.Thread(target=autodownloadAudioOnSDcard,args=(slaveDeviceWiFiDeviceNumber2,fileNumberToDownloadString,"slave_device_number_2",),)
...
def accessSDcard(masterDeviceNetworkForSDaccess,slaveDeviceNetworkForSDaccess,slaveDeviceNetworkForSDaccessDeviceNumber2,masterDeviceIPForSDaccess,slaveDeviceIPForSDaccess,slaveDeviceIPForSDaccessDeviceNumber2,tempSelectedSDcardFunctionToPerformValue):
    ...
    if tempSelectedSDcardFunctionToPerformValue == "1":
        ...
        generateJson(masterDeviceNetworkForSDaccess,slaveDeviceNetworkForSDaccess,slaveDeviceNetworkForSDaccessDeviceNumber2,masterDeviceIPForSDaccess,slaveDeviceIPForSDaccess,slaveDeviceIPForSDaccessDeviceNumber2,placeHolder,timeOfAccessingSDcard)
        listFilesOnSDcard()
...
# ================================================================================================================#
# add logic for multiple devices, also for slave device
def autodownloadSensorDataOnSDcard(deviceNetworkForSensorDataOnSDcard, fileNumberToDownload, sensorName, deviceFileName):
...
# ================================================================================================================#
def generateJson(masterDeviceNetworkForJson, slaveDeviceNetworkForJson,slaveDeviceNetworkForJsonDeviceNumber2, masterDeviceIPForJson,slaveDeviceIPForJson,slaveDeviceIPForJsonDeviceNumber2,fileNumberFromPythonScript,timeStampForJson):
...
def main():
    ...
    # Access SD card
        ...
        accessSDcard(masterDeviceEthernet,slaveDeviceEthernet,slaveDeviceEthernetDeviceNumber2,DEVICE_0_ETHERNET_IP,DEVICE_1_ETHERNET_IP,DEVICE_2_ETHERNET_IP,selectedSDcardFunctionToPerformValue)
