...
############################################################
            if deviceType == 0:
                audio_processing_queue.put(data)
############################################################
...
def process_audio_realtime():
    """
    Processes audio data in real time by consuming data from the audio_processing_queue.
    It accumulates enough bytes for a 100ms chunk, then applies a bandpass filter and beamforming,
    ...
    energy_map_initial = np.zeros((len(azimuth_range), len(elevation_range)))
    ...
    # Initialize a buffer to accumulate audio bytes.
    buffer_bytes = bytearray()

    # Main processing loop.
    while True:
        try:
            # Get next block of audio data from the queue (with timeout).
            data = audio_processing_queue.get(timeout=0.5)
            buffer_bytes.extend(data)
        except queue.Empty:
            # If no new data and recording is complete for device 0, exit.
            if doneReceivingAudio[0] and audio_processing_queue.empty():
                break
            else:
                continue

        # Process as many complete chunks as possible.
        while len(buffer_bytes) >= CHUNK_BYTES:
            chunk_data = buffer_bytes[:CHUNK_BYTES]
            buffer_bytes = buffer_bytes[CHUNK_BYTES:]
            ...
            # Identify the direction with maximum energy.
